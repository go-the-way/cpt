...
func (w *GzipResponseWriter) Write(b []byte) (int, error) {
	...
	// Save to write into a buffer for later use in GZIP responseWriter (if content is long enough) or at close with regular responseWriter.
		...
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if acceptsGzip(r) {
				...
				if _, ok := w.(http.CloseNotifier); ok {
					gww := GzipResponseWriterWithCloseNotify{gw}
					h.ServeHTTP(gww, r)
				} else {
					h.ServeHTTP(gw, r)
				}
...
// formatting errors.
func parseCoding(s string) (coding string, value float64, err error) {
	for n, part := range strings.Split(s, ";") {
		...
		if n == 0 {
			...
			value, err = strconv.ParseFloat(strings.TrimPrefix(part, "q="), 64)
